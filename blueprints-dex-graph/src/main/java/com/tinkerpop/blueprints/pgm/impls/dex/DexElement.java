 ...
 * <p/>
 * All elements are typed or labeled. The way to get the type or label for an element is retrieving the property
 * {@link DexElement#LABEL_PROPERTY}. This property cannot be set or removed.
 * <p/>
 * TODO: When a Float value is set for an attribute, it is stored as a Double value. Thus, when it is retrieved later,
 * it is retrieved as a Double and not as a Float.
 * 
...
public class DexElement implements Element {
	...
	protected String getTypeLabel() {
		return DexTypes.getTypeData(graph.getRawGraph(), getObjectType()).getName();
	}
	...
	public Object getProperty(final String key) {
		com.sparsity.dex.gdb.Attribute adata = DexAttributes
				.getAttributeData(graph.getRawGraph(), getObjectType(), key);
		if (adata == null) {
			return null;
		}
	...
	public void setProperty(final String key, final Object value) {
		...
		if (key.compareTo(LABEL_PROPERTY) == 0) {
			throw new UnsupportedOperationException(LABEL_PROPERTY + " property cannot be set.");
		}
	...
	public boolean hasId(Object identifier) {
		if (identifier instanceof Long) {
			return ((Long) identifier).equals(oid);
		}
		return false;
	}
	...
	public boolean equals(final Object object) {
		return (null != object)
				&& (this.getClass().equals(object.getClass()) && this.getId().equals(((Element) object).getId()));
	}
